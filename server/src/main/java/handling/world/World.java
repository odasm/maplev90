...
public class World {
    ...
    public static class Party {
        ...
        private static Map<Integer, MapleExpedition> expeditions = new HashMap<>();
    ...
    public static class randomWorldStuff {
        private static List<String> loggedOnSinceLastRestart = new ArrayList<>();
    ...
    public static class Family {
        ...
        private static final Map<Integer, Pair<Integer, Integer>> senReps = new LinkedHashMap<>(); // <chr id, <sen id, reps>>
        private static final Map<Integer, Pair<Integer, Integer>> sensenReps = new LinkedHashMap<>(); // <chr id, <sensen id, reps>>
        private static final Map<Integer, Integer> minusReps = new LinkedHashMap<>();
        public static long lastDayTime = 0;
        private static final ReentrantReadWriteLock lock = new ReentrantReadWriteLock();
        ...
        public static boolean canResetReps(long now) {
            return (lastDayTime > 0 && (lastDayTime + (1 * 24 * 60 * 60 * 1000) < now));
        }

        public static void resetReps() {
        ...
        public static int getSenRep(int cid, int senid) {
            lock.readLock().lock();
            try {
                final Pair<Integer, Integer> data = senReps.get(cid);
                if (data == null) {
                    return 0;
                }
                if (data.getLeft() == senid) {
        ...
        public static void setSenRep(int cid, int senid, int reps) {
            lock.writeLock().lock();
            try {
                if (cid == 0 || senid == 0) {
                    return;
                }
                senReps.put(cid, new Pair<>(senid, reps));
            } finally {
                lock.writeLock().unlock();
            }
        }

        public static int getSenSenRep(int cid, int sensenid) {
            lock.readLock().lock();
            try {
                final Pair<Integer, Integer> data = sensenReps.get(cid);
                if (data == null) {
                    return 0;
                }
                if (data.getLeft() == sensenid) {
                    return data.getRight();
                }
            } finally {
                lock.readLock().unlock();
        ...
        public static void setSenSenRep(int cid, int sensenid, int reps) {
            lock.writeLock().lock();
            try {
                if (cid == 0 || sensenid == 0) {
                    return;
                }
                sensenReps.put(cid, new Pair<>(sensenid, reps));
            } finally {
                lock.writeLock().unlock();
            }
        }

        public static int getMinusReps(int cid) {
            lock.readLock().lock();
            try {
                final Integer data = minusReps.get(cid);
                if (data == null) {
                    return 0;
                }
                return (int) data;
        ...
        public static void setMinusReps(int cid, int reps) {
            lock.writeLock().lock();
            try {
                if (cid == 0) {
                    return;
                }
                minusReps.put(cid, reps);
            } finally {
                lock.writeLock().unlock();
            }
        }
        ...
        public static void setFamilyMemberOnline(MapleFamilyCharacter mfc, boolean bOnline, int channel, long login) {
            MapleFamily f = getFamily(mfc.getFamilyId());
            if (f != null) {
                f.setOnline(mfc.getId(), bOnline, channel, login);
            }
        }

        public static int minusRep(int fid, int cid, int addrep) {
            MapleFamily f = getFamily(fid);
            if (f != null) {
                return f.minusRep(cid, addrep);
            }
            return 0;
        }

        public static int setRep(int fid, int cid, int addrep, int oldLevel, boolean sen, int sponser, int sponserSID, String oldName) {
            MapleFamily f = getFamily(fid);
            if (f != null) {
                return f.setRep(cid, addrep, oldLevel, sen, sponser, sponserSID, oldName);
            }
            return 0;
        }
        ...
        public static void setFamily(int familyid, int seniorid, int junior1, int junior2, int currentrep, int totalrep, int cid, int level, int job) {
            int ch = Find.findChannel(cid);
            if (ch == -1) {
                // System.out.println("ERROR: cannot find player in given channel");
                return;
            ...
            mc.setFamily(familyid, seniorid, junior1, junior2, level, job);
    ...
    public static class Respawn implements Runnable {
        ...
        private ArrayList<MonsterStatusEffect> effects = new ArrayList<>();
        private ArrayList<MapleMapItem> items = new ArrayList<>();
        private ArrayList<MapleCharacter> chrs = new ArrayList<>();
        private ArrayList<MapleMonster> mobs = new ArrayList<>();
        private ArrayList<MapleDiseaseValueHolder> dis = new ArrayList<>();
        private ArrayList<MapleCoolDownValueHolder> cd = new ArrayList<>();
        private ArrayList<MaplePet> pets = new ArrayList<>();
        ...
        if (isHotTimeStarted(now) && canTakeHotTime(chr.getName(), now) && chr.isAlive()) {
            if (chr.getLevel() > 30 && chr.getInventory(MapleInventoryType.USE).getNextFreeSlot() > -1 && chr.getConversation() == 0) {
                if (!chr.haveItem(2022336, 1, false, true)) {
                    ...
                    MapleInventoryManipulator.addById(chr.getClient(), 2022336, (short) 1, null, null, 7, "Obtained from Hot time Event on " + FileoutputUtil.CurrentReadable_Time());
