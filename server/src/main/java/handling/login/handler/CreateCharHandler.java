...
public class CreateCharHandler extends AbstractMaplePacketHandler{
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
		...
		for (int i = 0; i <= 4; i++) {
			...
			item.setInt((short) 1);
			item.setLuk((short) 1);
			item.setWatk((short) 1);
			item.setMatk((short) 1);
			item.setUpgradeSlots((byte) 0);
			item.setPosition(pos[i]);
			item.setGMLog("Obtained from character creation");
			equip.addFromDB(item);
		}
		IItem wea = li.getEquipById(weapon);
		wea.setPosition((byte) -11);
		wea.setGMLog("Obtained from character creation");
		equip.addFromDB(wea);
