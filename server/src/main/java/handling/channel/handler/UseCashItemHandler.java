...
public class UseCashItemHandler extends AbstractMaplePacketHandler {
	...
	public void handlePacket(SeekableLittleEndianAccessor slea, final MapleClient c) {
			...
			if (used) {
				...
				case 8192: // mp
					...
					if (job == 0) { // Beginner
						...
						if ((job >= 200 && job <= 232) || (GameConstants.isEvan(job)) || (job >= 3200 && job <= 3212)) { // Magician
		...
		case 5610000: { // Vega 30
			...
			used = InventoryHandlerUtils.UseUpgradeScroll(src, dst, (byte) 2, c, c.getPlayer(), itemId, (byte) 0); 
