         ...
         if (player != null && !player.isClone()) {
             if (!player.isGM() || c.getPlayer().isGM()) {
                 if (c.getPlayer().getId() == objectid) {
                     if (!player.getCustomQuestData(170001).equals("") && player.getPet(0) != null) {
                         c.getSession().write(PetPacket.loadExceptionList(player.getId(), player.getPet(0).getUniqueId(), player.getCustomQuestData(170001)));
                     }
                 }
