    ...
    public static void applyAttack(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, int attackCount, final double maxDamagePerMonster, final MapleStatEffect effect, final AttackType attack_type) {
        ...
        if (attack.skill != 0) {
            ...
            if (attack.targets > effect.getMobCount()) { // Must be done here, since NPE with normal atk
				if (!GameConstants.isVisitorSkill(attack.skill)) {
        ...
        if (attack.hits > attackCount) {
        	if(attack.skill != 21101003){
        		if (attack.skill != 4211006 && !GameConstants.isVisitorSkill(attack.skill)) {
    				player.getCheatTracker().registerOffense(CheatingOffense.MISMATCHING_BULLETCOUNT);
                    return;
                }
        	}
            ...
            if (monster != null) {
                ...
                if (totDamageToOneMonster > 0) {
                    if (attack.skill != 1221011) {
                        monster.damage(player, totDamageToOneMonster, true, attack.skill, attack.real);
                    } else {
                        monster.damage(player, (monster.getStats().isBoss() ? 199999 : (monster.getHp() - 1)), true, attack.skill, attack.real);
                    }
                    ...
                    switch (attack.skill) {
                        ...
                        case 5220011: {//homing
                            player.setLinkMid(monster.getObjectId(), effect.getX());
                            break;
                        }
        ...
        if (totDamage > 1) {
            ...
            if (tracker.getAttacksWithoutHit() > 1000 && !GameConstants.isVisitorSkill(attack.skill)) {
...
//	}
    ...
    public static final AttackInfo DivideAttack(final AttackInfo attack, final double rate) {
        attack.real = false;
        if (rate <= 1) {
            return attack;
        }
                    ...
                    for (Pair<Integer, Boolean> eachd : p.attack) {
                        ...
                        if (!eachd.right) {
                            if (attack.skill == 4221001) { //assassinate never crit first 3, always crit last
                            	...
                            	if( JobConstants.isEvanSkill(attack.skill)){//rouch fix
                            		CriticalRate = 35;
                            	}
    ...
    public static final AttackInfo parseDmgM(final LittleEndianAccessor lea) {
        ...
        int oid, damage;
        List<Pair<Integer, Boolean>> allDamageNumbers;
        boolean isAran = false;
        if(ret.targets == 0 && ret.skill == 21101003){
        	ret.targets = (byte)(ret.targets + 1);
        	isAran = true;
        }
