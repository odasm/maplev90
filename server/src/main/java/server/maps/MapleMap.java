...
public final class MapleMap {
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
        ...
        final int cmServerrate = (int) (ChannelServer.getInstance(channel).getMesoRate());
        final int chServerrate = (int) (ChannelServer.getInstance(channel).getDropRate());
        final int caServerrate = (int) (ChannelServer.getInstance(channel).getCashRate());
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * (chr.getStat().dropBuff / 100.0) * (showdown / 100.0))) {
            	byte questStatus = chr.getCustomQuestStatus(de.questid);
            	byte customQuestStatus = chr.getQuestStatus(de.questid);
            	if (questStatus > 1 || customQuestStatus > 1) {
                    continue;
                }
            	j++;
                if (droptype == 3) {
                ...
                if (de.itemId == 0) { // meso
						...
						spawnMesoDrop(mesos * cmServerrate, calcDropPos(pos, mob.getPosition()), mob, chr, false, droptype);
					    }
					    d++;
                } else {
                    int qqu = 0;
        ...
        for (final MonsterGlobalDropEntry de : globalEntry) {
            ...
            if (Randomizer.nextInt(999999) < de.chance && (de.continent < 0 || (de.continent < 10 && mapid / 100000000 == de.continent) || (de.continent < 100 && mapid / 10000000 == de.continent) || (de.continent < 1000 && mapid / 1000000 == de.continent))) {
                if (de.questid > 0 && ((chr.getCustomQuestStatus(de.questid) == 1 && CustomQuestProvider.getInstance(de.questid) == null) || (chr.getQuestStatus(de.questid) != 1) || (chr.getCustomQuestStatus(de.questid) != 1))) {
                    continue;
                }
                if (droptype == 3) {
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill, final boolean real) {  
        ...
        if (mobid == 8810018 && mapid == 240060200) { // Horntail
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "Expedition who defeated Pink Bean with invicible passion! You are the true timeless hero!"));
            for (MapleCharacter c : getCharactersThreadsafe()) {
    ...
    public List<MapleMonster> getAllMonstersThreadsafe() {
        return getAllMonstersThreadsafe(new ArrayList<MapleMonster>());
    }
            ...
            public final void sendPackets(MapleClient c) {
                ...
                if (monster.getId() == 9300166) {
                    MapTimer.getInstance().schedule(new Runnable() {
                        ...
                        @Override
                        public final void run() { // will this cause an npE if there're no chars on the map? :O
...
//		c.getSession().write(MobPacket.spawnFakeMonster(monster, 0));
        ...
        spawnAndAddRangedMapObject(door, new DelayedPacketCreation() {
            ...
            public final void sendPackets(MapleClient c) {
                ...
                if (door.getOwner().getParty() != null && (door.getOwner() == c.getPlayer() || door.getOwner().getParty().containsMembers(new MaplePartyCharacter(c.getPlayer())))) {
                    c.getSession().write(MapleUserPackets.partyPortal(door.getTown().getId(), door.getTarget().getId(), door.getSkill(), door.getTargetPosition()));
                }
    ...
    public List<MapleMapItem> getAllItemsThreadsafe() {
        return getAllItemsThreadsafe(new ArrayList<MapleMapItem>());
    }
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (chr.getParty() != null && !chr.isClone()) {
            ...
            chr.getClient().getSession().write(MapleUserPackets.updateParty(chr.getClient().getChannel(), chr.getParty(), PartyOperation.SILENT_UPDATE, null));
        ...
        if (mapid == 970030020) {
            chr.getClient().getSession().write(MaplePacketCreator.musicChange("Bgm10/Eregos"));
            if (chr.getJob() == 0 || chr.getJob() == 1000 || chr.getJob() == 2000 || chr.getJob() == 2001/* || chr.isGM()*/) {
                if (chr.getCustomQuestStatus(100000) != 2 /*|| chr.isGM()*/) { // refresh map 
                    NPCScriptManager.getInstance().start(chr.getClient(), 9000035);
                }
            }
        } else if (mapid == 502010000 && ((chr.getCustomQuestStatus(100000) == 2 && chr.getCustomQuestStatus(100001) == 0) || (chr.getCustomQuestStatus(100002) == 2 && chr.getCustomQuestStatus(100003) == 2 && chr.getCustomQuestStatus(100004) == 0))) {
            chr.cancelMorphs(true); // force cancel.
            NPCScriptManager.getInstance().dispose(chr.getClient());
            NPCScriptManager.getInstance().start(chr.getClient(), 9250129);
        } else if (mapid == 502010300) { // In miner map 
            NPCScriptManager.getInstance().dispose(chr.getClient());
            NPCScriptManager.getInstance().start(chr.getClient(), 9250125);
        /*} else if (mapid == 502021010 || mapid == 502022010 || mapid == 502029000 || mapid == 100000000 || mapid == 1000000 || mapid == 100030101 || mapid == 100030300) {
            ...
            ii.add(1300005);
            ii.add(2082010);
            chr.getClient().getSession().write(MaplePacketCreator.setNPCScriptable(ii));*/
    ...
    public final List<MapleCharacter> getCharactersThreadsafe() {
        return getCharactersThreadsafe(new ArrayList<MapleCharacter>());
    }

    public final ArrayList<MapleCharacter> getCharactersThreadsafe(ArrayList<MapleCharacter> chars) {
        chars.clear();
        charactersLock.readLock().lock();
        try {
            for (MapleCharacter mc : characters) {
    ...
    public void respawn(boolean force, long now) {
        ...
        if (force) { //cpq quick hack
            final int numShouldSpawn = (monsterSpawn.size() * ServerConstants.getRespawnRate(mapid)) - spawnedMonstersOnMap.get();
            if (numShouldSpawn > 0) {
                int spawned = 0;
            ...
            final int numShouldSpawn = (maxRegularSpawn * ServerConstants.getRespawnRate(mapid)) - spawnedMonstersOnMap.get();
            if (numShouldSpawn > 0) {
                int spawned = 0;
    ...
    public final boolean canSpawn(final long now) {
	...
	public void killAllBoogies() {
		...
		int boogies [] = new int[]{
				3230300, 3230301, 6130104, 9300050, 9400005, 9400006, 9400007, 9400008, 8800111	
		};
		...
		this.broadcastMessage(MaplePacketCreator.serverNotice(6,
				"As the rock crumbled, Jr. Boogie fell in great pain and disappeared."));
	}
