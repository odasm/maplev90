    ...
    public final void enterPortal(final MapleClient c) {
        ...
        if (portalState || c.getPlayer().isGM()) {
            if (getScriptName() != null) {
                ...
                if (to.getId() == 502010040) {
                    for (int i = 100000; i <= 100003; i++) {
                        if (c.getPlayer().getCustomQuestStatus(i) != 2) {
                            c.getPlayer().dropMessage(5, "A weird force is preventing you from entering the portal");
                            c.getSession().write(MaplePacketCreator.enableActions());
                            return;
                        }
                    }
					if ((!c.getPlayer().getCustomQuestData(100004).contains("step=01") && c.getPlayer().getCustomQuestStatus(100004) != 2) || c.getPlayer().getCustomQuestStatus(100005) == 2 && c.getPlayer().getCustomQuestStatus(100006) == 2 && c.getPlayer().getOccupation() > 0) {
						c.getPlayer().dropMessage(5, "A weird force is preventing you from entering the portal");
                        c.getSession().write(MaplePacketCreator.enableActions());
                        return;
					}
                } else if (to.getId() == 502010030) { // Dr bing's lab
                    if (c.getPlayer().getCustomQuestStatus(100005) != 2 || c.getPlayer().getCustomQuestStatus(100006) != 2 || c.getPlayer().getOccupation() <= 0) {
                        c.getPlayer().dropMessage(5, "Doctor Bing: Don't disturb me! I'm doing my research!");
                        c.getSession().write(MaplePacketCreator.enableActions());
                        return;
                    }
                } else if (to.getId() == 502010020) { // Base Central command
					if (c.getPlayer().getCustomQuestStatus(100009) == 0) {
						c.getPlayer().dropMessage(5, "A weird force is preventing you from entering the portal");
						c.getSession().write(MaplePacketCreator.enableActions());
						return;
					}
