...
public class MapleMapItem extends AbstractMapleMapObject {
	...
	public final boolean canLoot(final MapleClient client) {
		if (questid <= 0) {
			return true;
		} else if (CustomQuestProvider.getInstance(questid) != null) {
			return (client.getPlayer().getCustomQuestStatus(questid) == 1);
		} else if (client.getPlayer().getQuestStatus(questid) == 1) {
            return true;
        }
		return false;
	}
