...
public class MapleInventoryManipulator {
    ...
    public static boolean addByIdMinutes(MapleClient c, int itemId, short quantity, int minutes, String gmLog) {
        return addId(c, itemId, quantity, null, null, minutes, true, gmLog) >= 0;
    }
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, String gmLog) {
        return addId(c, itemId, quantity, owner, pet, period, false, gmLog) >= 0;
    }

    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, String gmLog) {
        return addId(c, itemId, quantity, owner, pet, period, false, gmLog);
    }

    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, boolean countInMinutes, String gmLog) {
    ...
    public static IItem addbyId_Gachapon(final MapleClient c, final int itemId, short quantity) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(itemId)) {
                ...
                while (quantity > 0) {
                    ...
                    if (newQ != 0) {
                        ...
                        nItem.setGMLog("Obtained from Gachapon on " + FileoutputUtil.CurrentReadable_Date());
            ...
            if (quantity == 1) {
                ...
                item.setGMLog("Obtained from Gachapon on " + FileoutputUtil.CurrentReadable_Date());
                final short newSlot = c.getPlayer().getInventory(type).addItem(item);
                if (newSlot == -1) {
                    return null;
                }
    ...
    public static boolean addFromDrop(final MapleClient c, IItem item, final boolean show, final boolean enhance) {
        ...
        c.getPlayer().havePartyQuest(item.getItemId());
        c.getPlayer().checkQuestDrops(item.getItemId());
        if (show) {
    ...
    private static final IItem checkEnhanced(final IItem before, final MapleCharacter chr) {
        if (before instanceof Equip) {
            ...
            int chance = 80;
            if (chr.getOccupation() == 305) { // Only level 5 hunter 
                chance = 75; // Additional 5% of gaining pot equips (total 25%) 
            }
            if (eq.getState() == 0 && (eq.getUpgradeSlots() >= 1 || eq.getLevel() >= 1) && Randomizer.nextInt(100) > chance) { //20% chance of pot?
                eq.resetPotential();
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if (/*!c.getPlayer().isGM() && */MapConstants.isStorylineMap(c.getPlayer().getMapId())) {
            if (GameConstants.isVisitorEquip(source.getItemId()) || source.getItemId() == 4031753 || source.getItemId() == 4031752 || source.getItemId() == 4001459) {
