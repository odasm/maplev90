...
public abstract class MapleEvent {
    ...
    public static final String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "Hello! Let's play a " + String.valueOf(event) + " event in channel " + cserv.getChannel() + "! Change to channel " + cserv.getChannel() + " and use @event command!"));
        return "";
    }
}
