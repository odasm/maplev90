...
public class CashShop implements Serializable {
    ...
    public IItem toItemWithQuantity(CashItemInfo cItem, int quantity, String gift) {
        return toItem(cItem, MapleInventoryManipulator.getUniqueId(cItem.getId(), null), gift, quantity, null);
    }

    public IItem toItem(CashItemInfo cItem) {
        return toItem(cItem, MapleInventoryManipulator.getUniqueId(cItem.getId(), null), "", 0, null);
    }

    public IItem toItem(CashItemInfo cItem, String gift) {
        return toItem(cItem, MapleInventoryManipulator.getUniqueId(cItem.getId(), null), gift, 0, null);
    }

    public IItem toItem(CashItemInfo cItem, int uniqueid) {
        return toItem(cItem, uniqueid, "", 0, null);
    }

    public IItem toItemWithLog(CashItemInfo cItem, String GMLog, String gift) {
        return toItem(cItem, MapleInventoryManipulator.getUniqueId(cItem.getId(), null), gift, 0, GMLog);
    }

    public IItem toItem(CashItemInfo cItem, int uniqueid, String gift, int quantity, String GMLog) {
        if (uniqueid <= 0) {
    ...
    public List<Pair<IItem, String>> loadGifts() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                IItem item = toItem(cItem, rs.getInt("uniqueid"), rs.getString("from"), 0, null);
