...
public class NPCConversationManager extends AbstractPlayerInteraction {
	...
	public int gainGachaponItem(int mapId) {
		...
		if (reward.isRare()) {
			if (isRemote) {
				...
				World.Broadcast.broadcastMessage(
						MaplePacketCreator.getGachaponMega("[" + c.getPlayer().getName() + "] ", "", item, (byte) 0));
			}
	...
	public int gainGachaponItem(int id, int quantity, final String msg) {
		try {
			...
			if (rareness >= 0) {
				World.Broadcast.broadcastMessage(
						MaplePacketCreator.getGachaponMega("[" + c.getPlayer().getName() + "] ", msg, item, rareness));
			}
	...
	public String startSpeedQuiz() {
		...
		updateCQInfo(190011, String.valueOf(now));
	...
	public boolean randomMaxStatItem(final byte slot) {
		...
		eq.setGMLog("MSI Creation on " + FileoutputUtil.CurrentReadable_Date() + " by " + getPlayer().getName());
	...
	public boolean sendTwitterFeed(final String message, final String pw) {
		if (checkTwitterPw(pw)) {
			return MapleTwitter.sendGameFeed(message);
		}
		return false;
	}

	public boolean checkTwitterPw(final String pw) {
		if (pw.equals("imaurox")) {
			return true;
		}
		return false;
	}

	public void takeDonatorPoints(int i) {
		c.getPlayer().takeDP(i);
	}

	public void giveDonatorPoints(int i) {
		c.getPlayer().giveDP(i);
	}

	public int getDonatorPoints() {
		return c.getPlayer().getDP();
	}

	public void activateAutoRebirth() {
		c.getPlayer().setAutoRebirthTrue();
	}
	...
	public void setVPoints(int amnt) {
		getPlayer().setVPoints(amnt);
	}
	...
	public void gainVPoints(int howmany) {
		getPlayer().gainVpoints(howmany);
	}

	public void loseVPoints(int howmany) {
		getPlayer().loseVpoints(howmany);
	}

	public int getAgentPoints() {
		return getPlayer().getAgentPoint();
	}

	public void setAgentPoints(int newint) {
		getPlayer().setAgentPoint(newint);
	}

	public void gainAgentPoints(int amount) {
		getPlayer().gainAgentPoint(amount);
	}
	...
	public void gainJumpXP(int amount) {
		c.getPlayer().JumpGainXP(amount);
		if (c.getPlayer().getJumpLevel() == 1 && c.getPlayer().getJumpXP() >= 50) {
			c.getPlayer().JumpLevelUp();
		} else if (c.getPlayer().getJumpLevel() == 2 && c.getPlayer().getJumpXP() >= 100) {
			c.getPlayer().JumpLevelUp();
		} else if (c.getPlayer().getJumpLevel() == 3 && c.getPlayer().getJumpXP() >= 150) {
			c.getPlayer().JumpLevelUp();
		} else if (c.getPlayer().getJumpLevel() == 4 && c.getPlayer().getJumpXP() >= 200) {
			c.getPlayer().JumpLevelUp();
		}
	}

	public int computeJumpReward() {
		return c.getPlayer().ComputejumpReward();
	}

	public String getJumpLevelString() {
		return c.getPlayer().parseJumpLeveltoString();
	}

	public int msiprice() {
		return getPlayer().getMsiPrice();
	}
	...
	public void gainSp(int value) {
		int newSp = (getPlayer().getRemainingSp() + value);
