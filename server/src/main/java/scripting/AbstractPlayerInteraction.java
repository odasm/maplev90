...
public abstract class AbstractPlayerInteraction {
    ...
    public final void warpAndSpawnMonster(final int map, final int monster, Point point) {
        final MapleMap mapz = getWarpMap(map);
        try {
            c.getPlayer().changeMap(mapz, mapz.getPortal(Randomizer.nextInt(mapz.getPortals().size())));
            spawnMob(monster, 1, point);
        } catch (Exception e) {
            c.getPlayer().changeMap(mapz, mapz.getPortal(0));
        }
    }
    ...
    public final int getPlayerStat(final String type) {
        ...
        } else if (type.equals("OCC")) {
			return c.getPlayer().getOccupation();
		} else if (type.equals("OCCEXP")) { // used for occ npc in occ town? :O show the progress bar [][][][][]
			return c.getPlayer().getOccupationEXP();
		}
        return -1;
    }
    ...
    public void setNpcScritable(final int npcId){
    	c.getSession().write(MaplePacketCreator.setNPCScriptable(Arrays.asList(npcId)));
    }
    ...
    public final void gainItem(final int id) {
    ...
    public final void gainItem(final int id, final short quantity, final boolean randomStats, final long period, final int slots, final String owner, final MapleClient cg) {
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) 
                ...
                item.setGMLog("Received from interaction " + oid + " (" + questid + ") on " + FileoutputUtil.CurrentReadable_Time());
                ...
                MapleInventoryManipulator.addById(cg, id, quantity, owner == null ? "" : owner, null, period, "Received from interaction " + oid + " (" + questid + ") on " + FileoutputUtil.CurrentReadable_Date());
            }
        } else {
    ...
    public final void givePartyItems(final int id, final short quantity, final List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            if (quantity >= 0) {
                MapleInventoryManipulator.addById(chr.getClient(), id, quantity, "Received from party interaction " + oid + " (" + questid + ")");
            } else {
    ...
    public void gainPet(int id, String name, int level, int closeness, int fullness, long period) {
        ...
        try {
            MapleInventoryManipulator.addById(c, id, (short) 1, "", MaplePet.createPet(id, name, level, closeness, fullness, MapleInventoryIdentifier.getInstance(), id == 5000054 ? (int) period : 0), 45, "Pet from interaction " + oid + " (" + questid + ")" + " on " + FileoutputUtil.CurrentReadable_Date());
    ...
    // Custom Quests
    public boolean startCQ(final int id) {
        return CustomQuestProvider.getInstance(id).start(c.getPlayer(), 0);
    }

    public boolean completeCQ(final int id) {
        return CustomQuestProvider.getInstance(id).complete(getPlayer(), 0);
    }

    public final byte getCustomQuestStatus(final int id) {
        return c.getPlayer().getCustomQuestStatus(id);
    }

    public final boolean isCQActive(final int id) {
        return getCustomQuestStatus(id) == 1;
    }

    public final boolean isCQFinished(final int id) {
        return getCustomQuestStatus(id) == 2;
    }

    public final String getCQInfo(final int id) {
        return c.getPlayer().getCustomQuestData(id);
    }

    public final void updateCQInfo(final int id, final String data) {
        final MapleCustomQuestStatus stat = new MapleCustomQuestStatus(CustomQuestProvider.getInstance(id), (byte) 1); // Always 1, since its in the middle of the quest
        stat.setCustomData(data);
