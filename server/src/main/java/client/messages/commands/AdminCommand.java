...
public class AdminCommand {
	...
	public static class Item extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (!c.getPlayer().isAdmin()) {
				/*
				 * for (int i : GameConstants.itemBlock) { if (itemId == i) {
				 * c.getPlayer().dropMessage(5,
				 * "Sorry but this item is blocked for your GM level."); return
				 * 0; } }
				 */
			}
		...
		public int execute(MapleClient c, String[] splitted) {
			if (!c.getPlayer().isHidden()) {
				...
				for (final MapleMapObject mmo : c.getPlayer().getMap().getAllMonstersThreadsafe()) {
					...
					c.getPlayer().getMap()
							.broadcastMessage(MobPacket.moveMonster(false, -1, 0, 0, 0, 0, monster.getObjectId(),
									monster.getPosition()/*
															 * , c.getPlayer().
															 * getPosition()
															 */, c.getPlayer().getLastRes()));
	...
	public static class GiveVPoint extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (chrs == null) {
				...
				c.getPlayer().dropMessage(6,
						splitted[1] + " has " + chrs.getVPoints() + " vpoints, after giving " + splitted[2] + ".");
			}
			return 1;
		}
	}
	...
	public static class Occupation extends CommandExecute {

		@Override
		public int execute(MapleClient c, String[] splitted) {
			c.getPlayer().changeOccupation(Short.parseShort(splitted[1]));
			c.getPlayer().dropMessage(6, "Success");
			return 1;
		}
	}

	public static class Tweet extends CommandExecute {

		@Override
		public int execute(MapleClient c, String[] splitted) {
			if (MapleTwitter.sendGameFeed("Welcome to CyntrixStory! I'm in game!")) {
				c.getPlayer().dropMessage(6, "Success");
			} else {
				c.getPlayer().dropMessage(6, "Failed.");
			}
			return 1;
		}
	}
				...
				for (int i = 0; i < amount; i++) {
					EtcTimer.getInstance().schedule(new Runnable() {
						public void run() {
							...
							toDrop.setGMLog(c.getPlayer().getName() + " used !AcidTrip, Amount: " + amount + " Delay: "
									+ delay);
	...
	public static class givedonorpoints extends CommandExecute {
		@Override
		public int execute(MapleClient c, String[] splitted) {
			if (splitted.length < 2) {
				World.Broadcast.broadcastMessage(
						MaplePacketCreator.getGameMessage(6, "Correct Syntax: !givedonorpoints <playername> <val>"));
			}
			MapleCharacter victim = null;
			victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
			if (victim != null) {
				victim.giveDP(Integer.parseInt(splitted[2]));
				victim.dcolormsg(7, "You have been given " + Integer.parseInt(splitted[2]) + "donator points.");
				c.getPlayer().dcolormsg(7,
						"You have given" + Integer.parseInt(splitted[2]) + "donator points to " + victim.getName());
			} else {
				c.getPlayer().dcolormsg(7, "Could not find the player.");
			}
			return 1;
		}
	}
