...
function action(m,t,s) {
    ...
    if (status == 0) {
        ...
        cm.sendNextS("#eYeap. So, whats so special here on NeoMS? I heard it su...", 3);
    } else if (status == 2) {
        cm.sendNextNPC("#eDude...", 5, 2131007);
    } else if (status == 3) {
        cm.sendNextS("#eThis "+gender+" is clueless. We are FULL of costum feature here on NeoMS. Let me give you a small glimpse of what you can expect.", 1);
    } else if (status == 4) {
        ...
        cm.sendNextS("#eAh. Anyway. As a reward for helping us, i'll give you a little pack with helpful items. Thanks for playing NeoMS. It is still in its very early ages! More feature will be developped soon.", 1);
    } else if (status == 43) {
        cm.getPlayer().dropMessage(1, "Please, if you like the server so far, go register on the forum right now and post an introduction. 'forum.NeoMS.info' or click the forum tab on the site.");
		cm.getPlayer().dropMessage(1, "Please, if you like the server so far, go register on the forum right now and post an introduction. 'forum.NeoMS.info' or click the forum tab on the site.");
		cm.warp(100000000, 0)
        cm.dispose();
    } else if (status == 50) {
