	...
	if (status == 0) {
		...
		if (cm.getJob() == 0) {
			if (cm.getPlayerStat("LVL") >= 10 && cm.getJob() == 0)
				...
				cm
						.sendOk("Train a bit more and I can show you the way of the #rThief#k.")
				cm.dispose();
			}
		} else {
			if (cm.getPlayerStat("LVL") >= 30 && cm.getJob() == 400) {
				if (cm.getQuestStatus(100009) >= 1) {
					...
					if (cm.getQuestStatus(100011) == 2) {
						status = 20;
						cm
								.sendNext("I see you have done well. I will allow you to take the next step on your long road.");
					} else {
				...
				if (cm.getQuestStatus(100101) == 2) {
					...
					cm
							.sendOk("Hey, #b#h0##k! I need a #bBlack Charm#k. Go and find the Door of Dimension.");
		...
		cm
				.sendNextPrev("It is an important and final choice. You will not be able to turn back.");
	} else if (status == 2) {
		...
		cm
				.sendNextPrev("You may be ready to take the next step as a #rAssassin#k or #rBandit#k.");
	} else if (status == 12) {
		cm
				.askAcceptDecline("But first I must test your skills. Are you ready?");
	} else if (status == 13) {
		cm.forceStartQuest(100009);
		cm.gainItem(4031011, 1);
		cm
				.sendOk("Go see the #bJob Instructor#k somewhere in the city. He will show you the way.");
		cm.dispose();
	} else if (status == 21) {
		cm
				.sendSimple("What do you want to become?#b\r\n#L0#Assassin#l\r\n#L1#Bandit#l#k");
	} else if (status == 22) {
		var jobName;
		if (selection == 0) {
			jobName = "Assassin";
