...
function action(mode, type, selection) {
	if (mode != 1) {
		...
		status++;
		if(status == 0){
			cm.sendNext("Hi There~ I'm Armi. It's the #b GM scroll event right now!#k right now, do you wish to plant the Maple Tree with me? With the Sunlight from #bMaple Leaf's#k it can allow the tree to grow healthy! Please gather all the Maple Leaf's that you get from the monsters...");
		} else if (status == 1) {
